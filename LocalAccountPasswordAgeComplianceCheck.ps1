...
$compsInput = $compsFilePath = $readFileOrManualEntryOrAllNodes = $outputMode = $userNamesFilter = $errors = $thisResult = `
...
#Prompt for filter by username
$userNamesFilter = read-host -prompt "`nFilter by username? Enter comma-seperated usernames or leave blank for all. [Default=All Users]"
If ($userNamesFilter -eq 'Q') { Exit }
...
$defaultOutFileName = "LocalUserPasswordAgeComplianceCheckOutput-$(Get-Date -Format MMddyyyy_HHmmss).csv"
...
$scriptblock1 = { 
    param($p1)
    If ($args[0] -or $p1) {
        If($args[0]) { $filter = $($args[0]).Split(",").Trim() }
        Else { $filter = $($p1).Split(",").Trim() }
        Get-LocalUser | Where-Object { $_.Name -in $filter } |
            ForEach-Object { 
                $user = $_
...
$comps | ForEach-Object { 
    $thisComp = $_
    If(!$defaultCredential -eq "N" -and !$localMachineOnly) { $thisResult = Invoke-Command -ComputerName $thisComp -Credential $Credential -ScriptBlock $scriptblock1 -ArgumentList $userNamesFilter -ErrorVariable errmsg 2>$null }
    ElseIf (!$localMachineOnly) { $thisResult = Invoke-Command -ComputerName $thisComp -ScriptBlock $scriptblock1 -ArgumentList $userNamesFilter -ErrorVariable errmsg 2>$null }
    Else { 
        $thisResult = &$scriptblock1 -p1 $userNamesFilter
        $thisResult | ForEach-Object { $_ | Add-Member -Name 'PSComputerName' -Type NoteProperty -Value $env:COMPUTERNAME }
    }
...
If ($results -and ($outputMode -eq 2 -or $outputMode -eq 3)) {
    ...
    $results | Select-Object @{n = 'Hostname' ; e = { $_.PSComputerName }},
                             ...
                             @{n = 'Last Logon' ; e = {$_.LastLogon}},
                             @{n = 'Is Enabled?' ; e = {$_.Enabled}},
                            ...
                            @{n = 'OS' ; e = { $_.OS }} | Sort-Object -Property @{e='Is Enabled?' ; descending=$true},@{e='Hostname'},@{e='Days Expired (+) or Until Expiration(-)' ; descending=$true} | 
...
If ($errors -and ($outputMode -eq 2 -or $outputMode -eq 3)) {
    ...
    If ($localhostErrors) { Write-Output "Try running script as administrator to address permissions issues on localhost query attempts." }
}
...
If ($outputMode -eq 1 -or $outputMode -eq 3 -and $results) {
    ...
    $results | Select-Object @{n = 'Hostname' ; e = { $_.PSComputerName }},
                            ...
                            @{n = 'Group Membership' ; e = { $_.Groups -join ", " }},
                            @{n = 'OS' ; e = { $_.OS }} | 
               Sort-Object -Property @{e='Is Enabled?' ; descending=$true},@{e='Hostname'},@{e='Days Expired (+) or Until Expiration(-)' ; descending=$true} | 
...
If ($outputMode -eq 1 -or $outputMode -eq 3 -and $errors) {
    ...
    If ($localhostErrors) { 
        $outputString = "`r`nTry running script as administrator to address permissions issues on localhost query attempts."
        Add-Content -Path $fileName -Value $outputString
    }
}
